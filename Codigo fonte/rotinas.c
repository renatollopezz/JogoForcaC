...
int i,teste,j=0;
		for(i=0;i<*cont;i++){
			 if(tolower(letraJogada) == letra[i] && *cont > 0 ){
		...
		for(i=0;i<1;i++){
			for(j=0;j<3;j++){
				for(k=0;k<strlen(palavras[i][j]);k++){
					tamanhoRespostas++;
					for(w=0;w<cont;w++){
						if (palavras[i][j][k]== tolower(letrasInformadas[w]) ){
...
int i,j,k,acertou =0;		
		for(i=0;i<1;i++){
			for(j=0;j<3;j++){
				for(k=0;k<strlen(palavras[i][j]);k++){
					if(palavras[i][j][k] == tolower(letraJogada)){
						acertou = 1;
					}
				}
			}
...
char letraJogada,letraJogada2;
	...
	do{
			...
			printf("\nInforme a letra:");
			scanf(" %c",&letraJogada);
			letrasInformadas[cont]= tolower(letraJogada);
